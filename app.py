...
with st.sidebar:
    ...
    st.markdown('<div style="font-size:12px; font-weight:700; color:rgba(255,255,255,0.4); margin-bottom:10px; text-transform:uppercase; letter-spacing:1px;">Navigation</div>', unsafe_allow_html=True)
    ...
    st.markdown('<div style="font-size:12px; font-weight:700; color:rgba(255,255,255,0.4); margin-bottom:10px; text-transform:uppercase; letter-spacing:1px;">Data Assets</div>', unsafe_allow_html=True)
...
with st.sidebar:
    st.markdown('<div style="font-size:12px; font-weight:700; color:rgba(255,255,255,0.4); margin-bottom:10px; text-transform:uppercase; letter-spacing:1px;">Analysis Filters</div>', unsafe_allow_html=True)
    ...
    st.markdown('<div style="margin: 20px 0;"></div>', unsafe_allow_html=True)
    st.markdown('<div style="font-size:12px; font-weight:700; color:rgba(255,255,255,0.4); margin-bottom:10px; text-transform:uppercase; letter-spacing:1px;">System Settings</div>', unsafe_allow_html=True)
    dark_mode = st.toggle("Dark Mode", value=True)
    st.markdown('<div style="font-size:11px; color:rgba(255,255,255,0.3); margin-top:20px;">v2.8 Enterprise Command Center</div>', unsafe_allow_html=True)
...
<style>
    ...
    [data-testid="stSidebar"] div[role="radiogroup"] label {{ font-size: 1.25rem !important; font-weight: 500 !important; padding: 14px 18px !important; border-radius: 16px !important; margin-bottom: 8px !important; transition: all 0.35s cubic-bezier(0.4, 0, 0.2, 1) !important; border: 1px solid transparent !important; background: rgba(255, 255, 255, 0.03) !important; display: flex !important; position: relative !important; overflow: hidden !important; }}
    [data-testid="stSidebar"] div[role="radiogroup"] label:hover {{ background: rgba(255, 255, 255, 0.1) !important; border: 1px solid rgba(255, 255, 255, 0.2) !important; transform: scale(1.02) translateX(5px) !important; box-shadow: 0 10px 20px rgba(0, 0, 0, 0.2) !important; }}
    [data-testid="stSidebar"] div[role="radiogroup"] label:has(input:checked) {{ background: linear-gradient(90deg, rgba(255, 255, 255, 0.15) 0%, rgba(255, 255, 255, 0.05) 100%) !important; border: 1px solid rgba(255, 255, 255, 0.3) !important; font-weight: 800 !important; box-shadow: 0 8px 30px rgba(0, 0, 0, 0.3) !important; }}
    [data-testid="stSidebar"] div[role="radiogroup"] label:has(input:checked)::before {{ content: ""; position: absolute; left: 0; top: 20%; height: 60%; width: 5px; background: #38bdf8; border-radius: 0 4px 4px 0; box-shadow: 0 0 15px rgba(56, 189, 248, 0.8); }}
    ...
    .kpi-card {{ border-radius: 20px; color: white !important; box-shadow: 0 8px 16px rgba(0,0,0,0.1); padding: 1.2rem; margin-bottom: 0.8rem; }}
...
st.session_state.avatar_b64 = get_base64_image("avatar/download.png")
...
# Global Reporting
with st.sidebar:
    st.markdown('<div style="margin: 20px 0;"></div><div style="font-size:12px; font-weight:700; color:rgba(255,255,255,0.4); margin-bottom:10px; text-transform:uppercase; letter-spacing:1px;">Global Reporting</div>', unsafe_allow_html=True)
